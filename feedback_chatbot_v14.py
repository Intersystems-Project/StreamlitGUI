...
def process_good_answers(llm_choice, api_key, question, answer):
    COLLECTION_NAME = "answers_embeddings"
    consolidated_ans = question + " This is the answer to the question: " + answer
    text_splitter = CharacterTextSplitter(chunk_size=250, chunk_overlap=45)
    docs = [Document(page_content=x) for x in text_splitter.split_text(consolidated_ans)]
    if llm_choice == 'Cohere':
        os.environ["COHERE_API_KEY"] = api_key
        embeddings = CohereEmbeddings(model="embed-english-v2.0", cohere_api_key=api_key)
    elif llm_choice == 'Google Gemini':
...
def ans_retrieve(llm_choice, api_key):
    COLLECTION_NAME = "answers_embeddings"
    if llm_choice == 'Cohere':
        os.environ["COHERE_API_KEY"] = api_key
        embeddings = CohereEmbeddings(model="embed-english-v2.0", cohere_api_key=api_key)
        ...
        os.environ["GOOGLE_API_KEY"] = api_key
        embeddings = GoogleGenerativeAIEmbeddings(model="models/embedding-001")
    db_answers = IRISVector(embedding_function=embeddings, connection_string=CONNECTION_STRING, collection_name=COLLECTION_NAME)
    return db_answers.as_retriever()
...
def execute_chain(query, api_key, llm_choice, user_input):
    ...
    Use ONLY these schema names which are (HIS_MedicalTrx, HIS_PatientTrx, HIS_Medical, HIS_System).
    ...
    ans_retriever = ans_retrieve(llm_choice, api_key)
    
    template = """Answer the question based only on the following context:
    {context}

    OR Answer the question by finding similar question and the corresponding answers based on this:
    {ans}
    ...
    retrieval_chain = (
        {"context": retriever, "ans": ans_retriever, "question": RunnablePassthrough()}
        | prompt
        | db_chain
    )
...
def clear_ans_vector(llm_choice, api_key):
    if llm_choice == 'Cohere':
        os.environ["COHERE_API_KEY"] = api_key
        embeddings = CohereEmbeddings(model="embed-english-v2.0", cohere_api_key=api_key)
    elif llm_choice == 'Google Gemini':
...
def main():
    ...
    with st.sidebar:
        ...
        st.button('🗑️ Clear history', on_click=lambda: clear_ans_vector(llm_choice, api_key))
            ...
            with st.spinner("Loading"):
                if user_question.startswith("plot"):
                    ...
                    if isinstance(ai_response, tuple) and len(ai_response) == 3:
                        ...
                        st.write(ai_response)
                        st.write("[Optional] Please rate the response:")
                        col1, col2, _ = st.columns([1,1,10], gap="small")
                        with col1:
                            st.button('👍', on_click=process_good_answers(llm_choice, api_key, user_question, ai_response))
                        with col2:
                            st.button('👎')
                        st.session_state.messages.append({"role": "assistant", "content": ai_response})
                    else:
                        ...
                        col1, col2, _ = st.columns([1,1,10], gap="small")
                        with col1:
                            st.button('👍', on_click=lambda: process_good_answers(llm_choice, api_key, user_question, ai_response))
                        with col2:
                            st.button('👎')
                        st.session_state.messages.append({"role": "assistant", "content": ai_response})
