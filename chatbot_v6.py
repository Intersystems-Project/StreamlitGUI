...
def rag_retrieve(file_path, collection_name):
    ...
    if COLLECTION_NAME in metadata.tables:
        db_rag = IRISVector(embedding_function=gemini_embeddings, connection_string=CONNECTION_STRING, collection_name=COLLECTION_NAME)
    else:
...
def execute_chain(query, api_key, llm_choice):
    ...
    db_chain = SQLDatabaseChain.from_llm(llm=llm, db=db_sql, prompt=PROMPT, verbose=False, return_intermediate_steps=True)
    
    retriever = rag_retrieve("factsheet_example.txt", "factsheet_test_gemini")
